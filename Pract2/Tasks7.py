    ...
    def printRoomData(self):
        ...
        for index in range(0, len(self.directions)):
            if self.directions[index][0] == "UP":
                print(index + 1, "\b. Проход на Север.", self.directions[index][2])
            elif self.directions[index][0] == "DOWN":
                print(index + 1, "\b. Проход на Юг.", self.directions[index][2])
            elif self.directions[index][0] == "RIGHT":
                print(index + 1, "\b. Проход на Восток.", self.directions[index][2])
            elif self.directions[index][0] == "LEFT":
                print(index + 1, "\b. Проход на Запад.", self.directions[index][2])
            else:
...
def levelConfiguration():
    ...
    rooms = [RoomConfig("Комната #1", "Вы стоите у высоких, древних ворот, "
             ...
             RoomConfig("Комната #16", "Вы находитесь в комнате, где стены покрыты кристаллами, "
                         ...
                         dir_desc=["Путь обратно к порталу. (#15)"])]  # goto 15
    return rooms
